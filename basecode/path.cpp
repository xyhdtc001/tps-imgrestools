...
Path Path::MakeRelativePath( const Path &makeRelative, const Path &relativeTo, uint32 mode )
{
   ...
   uint32 minDirCount = getMin(makeRelative.getDirectoryCount(), relativeTo.getDirectoryCount());
   ...
   for (uint32 i = divergeDirIdx; i < makeRelative.getDirectoryCount(); i++)
   ...
   uint32 numOffset = relativeTo.getDirectoryCount() - divergeDirIdx;

   // Push back an "up" for each directory we are offset
   for (uint32 i = 0; i < numOffset; i++)
      offsetPath += "../";
...
uint32 Path::getDirectoryCount() const
{
   if (mPath.isEmpty())
      return 0;
...
String Path::getDirectory(uint32 count) const
{
   if (mPath.isEmpty())
      return String();
