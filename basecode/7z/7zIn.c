	...
	const char* password)
{
  ...
  res = SzFolder_Decode(folder, p->PackSizes,
          ...
          outBuffer->data, (size_t)unpackSize, allocTemp, password);
  RINOK(res);
    ...
    if (CrcCalc(buffer.data, nextHeaderSizeT) == nextHeaderCRC)
    {
      ...
      if (res == SZ_OK)
      {
        if (type == k7zIdEncodedHeader)
        {
          CBuf outBuffer;
          Buf_Init(&outBuffer);
          res = SzReadAndDecodePackedStreams(inStream, &sd, &outBuffer, p->startPosAfterHeader, allocTemp, password);
          if (res != SZ_OK)
...
SRes SzArEx_Open(CSzArEx *p, ILookInStream *inStream, ISzAlloc *allocMain, ISzAlloc *allocTemp, const char* password)
{
  SRes res = SzArEx_Open2(p, inStream, allocMain, allocTemp, password);
  if (res != SZ_OK)
      ...
      *outBufferSize = unpackSize;
      ...
      if (res == SZ_OK)
      {
        res = SzFolder_Decode(folder,
          ...
          *outBuffer, unpackSize, allocTemp, password);
        if (res == SZ_OK)
        {
          if (folder->UnpackCRCDefined)
          {
