...
int GetUnpackSize_7z(const char *file, const char *data, const char *password)
{
	...
	if (InFile_Open(&archiveStream.file, data))
	{
		return -1;
	}
	...
	SzArEx_Init(&db);
	res = SzArEx_Open(&db, &lookStream.s, &allocImp, &allocTempImp, password);
	if (res == SZ_OK)
	{
		if (res == SZ_OK)
		{
...
//				size_t outSizeProcessed = 0;
				...
				if (strcmp(file, (const char *)buf.data))
				continue;
				Buf_Free(&buf, &g_Alloc);
...
int DecodeToBuffer_7z(void *buf, int size, const char *file, const char *data, const char* password)
{
	CFileInStream archiveStream;
	CLookToRead lookStream;
	CSzArEx db;
	...
	if (res == SZ_OK)
	{
		if (res == SZ_OK)
		{
			...
			for (i = 0; i < db.db.NumFiles; i++)
			{
				...
				Buf_Init(&bufff);
				res = Utf16_To_Char(&bufff, temp, 0);
				if (!strcmp(file, (const char *)bufff.data))
				{
					res = SzArEx_Extract(&db, &lookStream.s, i,
						&blockIndex, &outBuffer, &outBufferSize,
						&offset, &outSizeProcessed,
						&allocImp, &allocTempImp, password);

					if (outSizeProcessed <= (size_t)size)
						size = (int)outSizeProcessed;

					memcpy(buf, outBuffer + offset, size);
					Buf_Free(&bufff, &g_Alloc);
					break;
				}
				Buf_Free(&bufff, &g_Alloc);
			}
...
ref_ptr<MemStream> DecodeToMemStream_7z( const char *file, const char *data, const char* password )
{
	...
	if ( res != SZ_OK )
	{
		LogErr( RES, "Pack open err %s in %s", file, data );
		return NULL;
	}

	if ( db.FileIndexToFolderIndexMap[*db.FolderStartFileIndex] == db.FileIndexToFolderIndexMap[*db.FolderStartFileIndex+1] )
	{
		//SzArEx_Free(&db, &allocImp);
		//SzFree(NULL, temp);
		//File_Close(&archiveStream.file);
		//return 0;
		//LogOut( RES, LOG_ERR, "πÃ µ£∫%s", data );
		//char szInfo[256] = "";
		//_snprintf( szInfo, 255, "πÃ µ£∫%s\n", data );
	}
		...
		if ( res == SZ_OK )
		{
			...
			for ( i = 0; i < db.db.NumFiles; i++ )
			{
				...
				if ( !strcmp(file, (const char *)bufff.data) )
				{
					...
					outStream = new ( outSizeProcessed )MemStream( (uint32)outSizeProcessed );
					memcpy(outStream->_data, outBuffer + offset, outSizeProcessed);
					Buf_Free(&bufff, &g_Alloc);
					break;
				}
				Buf_Free(&bufff, &g_Alloc);
